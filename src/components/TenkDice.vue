...
function checkPoints(nums: number[]) {
  ...
  nums.forEach(x => {
    if (x == 1 && !remove1.value) tempPoints += 100;
    else if (x == 5 && !remove5.value) tempPoints += 50;
  })
  if (tempPoints == 0) {
    fail.value = true;
    failedTurn();
...
function stopReRoll(e: number) {
  ...
  if (allUsed == 0) {
    setTimeout(resetDice, 500, false);
  }
}
  ...
  <div class="wrapper">
    <div class="dice-field">
      <div class="pair" v-for="d in dice" :key="d.value.name">
        ...
        <button @click="stopReRoll(d.value.n_value)" v-if="d.value.reroll">Save</button>
      </div>
    </div>
  </div>
  <div id="reroll">
